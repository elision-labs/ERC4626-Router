...
contract MockVaultV3 is ERC4626 {
    ...
    constructor(ERC20 underlying) ERC4626(underlying, "Mock Helix4626", "yMTKN") {}
    ...
    function withdraw(uint256 assets, address receiver, address owner, uint256 maxLoss) public returns (uint256) {
    ...
    function withdraw(
        ...
        while (totalAsset() < assets && i < _strategies.length) {
            ...
            uint256 toWithdraw = mockStrategy.maxWithdraw(address(this)) > assets
                ? assets
                : mockStrategy.maxWithdraw(address(this));
    ...
    function redeem(
        ...
        while (totalAsset() < shares && i < _strategies.length) {
            MockStrategy mockStrategy = MockStrategy(_strategies[i]);
            uint256 toRedeem = mockStrategy.maxRedeem(address(this)) > shares
                ? shares
                : mockStrategy.maxRedeem(address(this));
